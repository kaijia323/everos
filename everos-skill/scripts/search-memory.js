#!/usr/bin/env node
/**
 * search-memory.js — 搜索 EverOS 记忆
 *
 * 用法:
 *   node search-memory.js --user-id <id> --query "搜索内容" \
 *     [--project-id <id>] [--app-id default] [--server http://localhost:18000] \
 *     [--method hybrid|keyword] [--top-k 5]
 *
 * 环境变量:
...
const API_PATH = '/api/v1/memory/search';
...
选项:
  ...
  --method           搜索方法: "hybrid" 或 "keyword" (默认: "hybrid")
  --top-k            返回结果数量 (默认: 5)
  --help, -h         显示帮助
...
示例:
  node search-memory.js --user-id user --query "项目计划" \\
    --project-id myapp --method hybrid --top-k 5
`);
}
...
async function main() {
  ...
  const topK = parseInt(opts['top-k'] || opts.top_k || '5', 10);
